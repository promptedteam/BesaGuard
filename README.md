...
Download **BesaGuard-Setup-0.8.13-win-x64.exe** from the [0.8.13 beta release](https://github.com/promptedteam/BesaGuard/releases/tag/v0.8.13), verify its SHA-256 against `SHA256SUMS-0.8.13.txt`, and run the installation wizard.
...
The 0.8.13 client uses a detached P-256 signed-v2 manifest and verifies the
exact metadata before using its version, GitHub URLs, size, or SHA-256. The
original feed remains only so already-installed 0.8.12 clients can discover
this migration release; it cannot authorize future downloads in the hardened
0.8.13 path. This metadata signature is separate from future Authenticode
signing of binaries.
...
- Automatic startup checks against the detached signed-v2 beta manifest, a toolbar download indicator shown only for a newer authenticated build, and SHA-256-verified installer downloads. After download, **Install update** asks for confirmation, rechecks the exact installer size and SHA-256, starts the interactive Setup wizard, and closes BesaGuard. Installers are never silently executed.
